...
**調査時点（2026-09-18 朝）ではコードを変えないと提出できなかった。
targetSdk 36 化は同日 1.1.0 で対応済み。以下は当時の記述をそのまま残す。**
...
- [x] Google Play デベロッパー アカウントを作成（登録料 **US$25**・1 回限り）（2026-09-18 完了）
      ※ 既にアカウントをお持ちなら、以下の本人確認とクローズドテスト要件は
      アカウント作成日が 2023-11-13 より前なら不要な可能性がある（§2 で分岐を説明）
- [x] 本人確認（氏名・正式な住所・電話番号・身分証）を完了させる（2026-09-18 完了）
...
- [x] **targetSdk / compileSdk を 36 へ**（必須。これが無いと受け付けられない）（2026-09-18 完了）
- [x] それに伴い **AGP 8.5.2 → 8.9.1 以上**、**Gradle 8.9 → 8.11.1 以上**（§1）（2026-09-18 完了: AGP 8.9.3 / Gradle 8.11.1。Kotlin 1.9.24 のままで通った）
- [ ] Android 16 の挙動変更への追随を実機で確認（edge-to-edge 強制、予測バック、
      sw600dp 以上での向き固定無視。§1 に本アプリへの影響の具体的な評価）
      → DECISIONS.md の H-08。**実発信は作者にしかできない**
- [x] **アプリ内にプライバシー ポリシーへのリンクを置く**（2026-09-18 完了）
      — Play は「Console のフィールド」と「アプリ内」の**両方**を要求している。
      詳細設定の末尾「このアプリについて」から開く。URL は `AppLinks` に 1 か所。§5
- [x] PRIVACY.md に**デベロッパー名を明記**する（2026-09-18 完了: `tmlksu`）。
      ストア掲載のエンティティ名がポリシー本文に出ていることが要件（§5）
- [x] `versionCode` を 2 以上に上げる（1 は GitHub 版 1.0.0 で使用済み。§8）（2026-09-18 完了: `versionCode = 2` / `versionName = "1.1.0"`）

### 推奨

- [x] **`android:allowBackup` の扱いを決める**（2026-09-18 決定: 案 B を実装済み）。現状 `true` のため、発信記録
...
- [ ] ストア説明に「Play 版には通話履歴の書き換えが無い」旨を明記（§9）
      ※ アプリ内の案内は 2026-09-18 に実装済み（詳細設定の該当位置に 1 行）
...
**対応済み（2026-09-18 / 1.1.0）。** 現在は `compileSdk = 36` / `targetSdk = 36`、
AGP 8.9.3 / Gradle 8.11.1。Kotlin 1.9.24 / Compose Compiler 1.5.14 /
Compose BOM 2024.06.00 は据え置きで通った（下の「Kotlin 2.0 へ上げることになる」
分岐は踏まずに済んだ）。以下は調査当時の記述。

調査時点の `app/build.gradle.kts`:
...
→ B を採用（2026-09-18 決定）。発信記録の SharedPreferences のみ
`fullBackupContent` / `dataExtractionRules` で除外し、設定は引き継ぐ。
...
| # | 判断事項 | 推奨 | 理由 |
|---|---|---|---|
| **P-01** | **`android:allowBackup` を `false` にするか** | **決定（2026-09-18）: 案 B を実装済み**。発信記録の SharedPreferences のみ `fullBackupContent` / `dataExtractionRules` で除外 | 現状 `true` だと発信記録（＝発信した番号）と設定がユーザーの Google ドライブへ行く。PRIVACY.md の「端末の外へ出ることはありません」と整合しない。JSON エクスポートがあるので機種変時の移行手段は確保されている。**安全側に倒すという本プロジェクトの原則そのまま。§4 / §5** |
